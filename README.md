...
開発ダッシュボードを全プロジェクトで標準生成します。判断待ち、現在・次順・保留、タスク、任意の画面画像/動画を正本から表示します。既存案件は `sync` で導入できます。入力形式とGit方針は [共通ダッシュボード](skills/hotl-gpt/references/dashboard.md) を参照してください。
